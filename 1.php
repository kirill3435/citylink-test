...
function searchWorker($area = ''):?string {
    global $areas, $nearby, $workers;
    $workersAreas = array_column($workers, 'area_name');
    
    $workerKey = array_search($area, $workersAreas);
    if ($workerKey) { 
       return $workers[$workerKey]["login"];
    } else {
        $areaKey = array_search($area, $areas);
        if ($areaKey) {
            $nearbyAreasKeys = $nearby[$areaKey];
            foreach ($nearbyAreasKeys as $val) {
                $NearbyAreasNames[] = $areas[$val];
            }
            $arrNearbyWorkerAreas = array_intersect($NearbyAreasNames, $workersAreas);
            if (!empty($arrNearbyWorkerAreas)) {
                $nearbyWorkerArea = array_shift($arrNearbyWorkerAreas);
                $workerKey = array_search($nearbyWorkerArea, $workersAreas);
